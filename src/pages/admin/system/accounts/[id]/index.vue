<script setup lang="ts">
  ...
  import { Account, SystemResponse } from '@/types'
  ...
  const loadUser = async (id: string) => {
    try {
      const response = await axios.get<SystemResponse<Account>>(
        `http://localhost:3200/api/system/accounts/${id}`
      )
  ...
  const handleSubmit = async () => {
    try {
      if (isEdit.value) {
        await axios.put(
          `http://localhost:3200/api/system/accounts/${(route.params as UserRouteParams).id}`,
          form.value
        )
        alert('User updated successfully!')
      } else {
        await axios.post('http://localhost:3200/api/system/accounts', form.value)
        alert('User added successfully!')
      }
      router.push('/admin/system/accounts')
    } catch (error) {
...
<template>
  <VContainer>
    <div class="d-flex justify-space-between align-center mb-4">
      ...
      <VBtn color="secondary" :to="'/admin/system/accounts'">
        Back to Accounts
      </VBtn>
    </div>
