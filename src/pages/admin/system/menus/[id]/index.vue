<template>
  <VContainer fluid>
    <div class="d-flex justify-space-between align-center mb-4">
      ...
      <VBtn color="secondary" :to="'/admin/system/menus'">Back to Menus</VBtn>
    </div>
    <VForm @submit.prevent="handleSubmit">
      ...
      <IconPicker v-model:dialog="iconPickerDialog" v-model:icon="form.icon" />
...
<script setup lang="ts">
  ...
  const loadMenu = async (id: string) => {
    try {
      const response = await axios.get(
        `http://localhost:3200/api/system/menus/${id}`
      )
  ...
  const handleSubmit = async () => {
    try {
      if (isEdit.value) {
        await axios.put(
          `http://localhost:3200/api/system/menus/${(route.params as MenuRouteParams).id}`,
          form.value
        )
