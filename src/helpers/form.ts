...
export const validationRules = {
  username: [(v: string) => !!v || 'Username is required', (v: string) => v.length >= 6 || 'Username must be at least 6 characters long', (v: string) => v.length <= 10 || 'Username must be at most 10 characters long', (v: string) => /^[a-zA-Z0-9_]+$/.test(v) || 'Username can only contain letters, numbers, and underscores', (v: string) => !/\s/.test(v) || 'Username cannot contain spaces'],
