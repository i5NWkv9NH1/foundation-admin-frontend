...
export const validationRules = {
  username: [
    ...
    (v: string) =>
      v.length <= 10 || 'Username must be at most 10 characters long',
    (v: string) =>
      /^[a-zA-Z0-9_]+$/.test(v) ||
  ...
  password: [
    ...
    (v: string) =>
      v.length <= 20 || 'Password must be at most 20 characters long',
  ...
  captcha: [
    (v: string) => !!v || 'Captcha is required',
    (v: string) => v.length === 4 || 'Captcha must be 4 characters long'
  ]
};
